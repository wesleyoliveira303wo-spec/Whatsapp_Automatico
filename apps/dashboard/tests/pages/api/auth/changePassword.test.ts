...
import { setSessionCookie, readSessionFromRequest, SESSION_COOKIE_NAME } from '../../../../lib/dashboardSession';
...
describe('POST /api/auth/change-password', () => {
  ...
  function userCookie(): string {
    return cookieFor({
      ...
      refreshToken: 'ref-1',
      user: { id: 'user-1', email: 'maria@empresa.com', role: 'operator', mustChangePassword: true },
    });
  }
  ...
  it('sessao de MAQUINA (API key): 403 user_session_required — nao ha senha para trocar', async () => {
    ...
    const req = createFakeReq({ method: 'POST', cookies: { [SESSION_COOKIE_NAME]: cookie }, body: BODY });
    const res = createFakeRes();

    await handler(req, res);

    expect(res.status).toHaveBeenCalledWith(403);
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: 'user_session_required' }));
  });
  ...
  it('caminho feliz: troca na API, RELOGA com a senha nova, regrava o cookie (mustChangePassword false) e responde 204', async () => {
    ...
    const req = createFakeReq({ method: 'POST', cookies: { [SESSION_COOKIE_NAME]: userCookie() }, body: BODY });
    const res = createFakeRes();
    ...
    expect(JSON.parse(calls[1][1].body as string)).toEqual({ email: 'maria@empresa.com', password: 'senha-nova-forte' });

    // Cookie regravado: refresh novo e post-it de senha provisoria removido.
    const rewritten = (res._headers['Set-Cookie'] as string).match(new RegExp(`^${SESSION_COOKIE_NAME}=([^;]*)`))![1];
    const session = readSessionFromRequest(createFakeReq({ cookies: { [SESSION_COOKIE_NAME]: rewritten } }));
