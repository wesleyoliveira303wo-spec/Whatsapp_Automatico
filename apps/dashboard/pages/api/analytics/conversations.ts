 ...
 * `pages/api/sessions/[sessionName]/analytics/conversations.ts`. Mesmo
...
export default function handler(_req: NextApiRequest, res: NextApiResponse): void {
  res.status(410).json({
    error: 'route_moved',
    message:
      'Esta rota foi substituída por /api/sessions/:sessionName/analytics/conversations (Analytics por sessão).',
  });
}
