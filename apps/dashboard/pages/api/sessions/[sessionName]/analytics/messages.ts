...
/**
 * Proxy para `GET .../sessions/:sessionName/analytics/messages` — migrado da
 * rota flat `pages/api/analytics/messages.ts` (M4D) para aninhado por sessão
 * (M6H-4, 2026-07-26). Mesmo padrão proxy-fino de `ai-usage.ts`.
 */
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  ...
  const { status, body } = await callAnalyticsApi(
    session,
    `/${encodeURIComponent(sessionName)}/analytics/messages`,
    {
