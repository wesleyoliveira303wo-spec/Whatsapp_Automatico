...
/**
 * Proxy para `GET .../sessions/:sessionName/analytics/session-stability` —
 * migrado da rota flat `pages/api/analytics/session-stability.ts` (M4D) para
...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  ...
  const { status, body } = await callAnalyticsApi(
    session,
    `/${encodeURIComponent(sessionName)}/analytics/session-stability`,
    {
