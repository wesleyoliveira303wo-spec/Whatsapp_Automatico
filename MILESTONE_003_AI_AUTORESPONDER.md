...
```ts
type ReplyValidationResult = { valid: true; sanitized: string } | { valid: false; reason: string };
```
...
_(inalterado da Revisão 1)_ `WhatsAppProviderEvent` +`message_received`; `WhatsAppProvider` +`sendMessage()`; `BaileysProvider` implementa ambos com os filtros de `fromMe`/grupo; `SessionManager` ganha dependência opcional `MessageReceivedHandler`.
...
_(riscos da Revisão 1 continuam válidos — reproduzidos + novos desta revisão)_
...
_(critérios da Revisão 1 continuam válidos + novos abaixo)_
