...
1. The user needs a hosting plan (this is the moment the honest-costs talk's hosting bullet gets said, if it has not come up yet). When they have one, they usually paste the connector command from Hostinger's connector page (its Install Extension button, with Claude Code selected, shows the command to copy). If they just say hosting is ready, run it yourself; it is: Windows `claude mcp add hostinger -- npx.cmd -y hostinger-api-mcp`, Mac/Linux the same with `npx`. The official Hostinger page lists one command with plain `npx` for every platform. Read a pasted command as intent and adapt it to the machine from context: on Windows that means the `npx.cmd` form. No commentary needed.
...
Have the user test the live site on desktop AND their phone, on the real network, not just localhost. Ask them to check the scroll at the top and the bottom of the hero specifically, in Chrome, where choppiness shows first. Their eyes and their hardware catch what audits cannot.
